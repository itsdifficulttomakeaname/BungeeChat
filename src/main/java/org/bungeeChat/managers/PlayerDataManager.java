...
public class PlayerDataManager {
    ...
    private static final int MESSAGE_EXPIRE_HOURS = 24; // 消息保存24小时
    private int messageExpireHours;

    public PlayerDataManager(BungeeChat plugin) {
        this.plugin = plugin;
        this.messageExpireHours = plugin.getConfigManager()
                .getConfig()
                .getInt("MessageSettings.expire_hours", 24);
        loadPlayerData();
    }
    ...
    public void cleanupExpiredMessages() {
        long expireTime = System.currentTimeMillis() -
                TimeUnit.HOURS.toMillis(messageExpireHours);
        messageHistory.entrySet().removeIf(entry ->
                entry.getValue().timestamp < expireTime // 使用正确的时间戳字段
        );
    }
